...
class CyberpunkBackground:
    """Procedural animated background in a cyberpunk style (parallax city + neon scanlines)."""
    def __init__(self, width: int, height: int):
        self.width = width
        self.height = height
        # Parallax layers: arrays of building rects (x, w, h, speed, color)
        random.seed(42)
        self.layers: list[list[tuple[float, float, float, float, tuple[int,int,int]]]] = []
        self.layer_speeds = [10.0, 22.0, 36.0]
        self.layer_colors = [(30, 20, 40), (45, 30, 60), (65, 40, 90)]
        self._build_layers()
        # Neon rain particles (x, y, speed, length)
        self.rain: list[tuple[float, float, float, float]] = []
        self._spawn_rain(140)
        # Stars
        self.stars: list[tuple[float, float, float]] = []  # x, y, speed
        for _ in range(80):
            self.stars.append((random.uniform(0, self.width), random.uniform(0, self.height * 0.6), random.uniform(4.0, 14.0)))
        # Scanlines surface
        self.scan_surface = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
        for y in range(0, self.height, 2):
            pygame.draw.line(self.scan_surface, (255, 0, 128, 10), (0, y), (self.width, y))
        self.time = 0.0
    ...
    def _build_layers(self) -> None:
        ...
        for li in range(3):
            layer: list[tuple[float, float, float, float, tuple[int,int,int]]] = []
            x = 0.0
            while x < self.width * 1.5:
                w = random.uniform(40, 120) * (1 + 0.2 * li)
                h = random.uniform(60, 180) * (1 + 0.35 * li)
                color = self.layer_colors[li]
                speed = self.layer_speeds[li]
                layer.append((x, w, h, speed, color))
                x += w + random.uniform(10, 30)
            self.layers.append(layer)

    def _spawn_rain(self, count: int) -> None:
        for _ in range(count):
            x = random.uniform(0, self.width)
            y = random.uniform(-self.height, self.height)
            speed = random.uniform(120.0, 320.0)
            length = random.uniform(6.0, 14.0)
            self.rain.append((x, y, speed, length))

    def update(self, dt: float) -> None:
        self.time += dt
        # Move stars slowly to the left
        updated_stars: list[tuple[float, float, float]] = []
        for x, y, s in self.stars:
            x -= s * 0.05 * dt
            if x < -2:
                x = self.width + random.uniform(0, 40)
                y = random.uniform(0, self.height * 0.6)
            updated_stars.append((x, y, s))
        self.stars = updated_stars

        # Move layers left to right for subtle parallax
        for li, layer in enumerate(self.layers):
            speed = self.layer_speeds[li]
            for idx in range(len(layer)):
                x, w, h, spd, color = layer[idx]
                x -= speed * dt
                if x + w < -20:
                    # recycle to the right
                    x = self.width + random.uniform(10, 40)
                    w = random.uniform(40, 120) * (1 + 0.2 * li)
                    h = random.uniform(60, 180) * (1 + 0.35 * li)
                layer[idx] = (x, w, h, spd, color)

        # Rain
        new_rain: list[tuple[float, float, float, float]] = []
        for x, y, s, L in self.rain:
            y += s * dt
            x += s * 0.06 * dt
            if y > self.height + 10:
                y = random.uniform(-self.height * 0.3, -10)
                x = random.uniform(0, self.width)
            new_rain.append((x, y, s, L))
        self.rain = new_rain

    def _draw_gradient(self, surface: pygame.Surface) -> None:
        # Vertical neon gradient background
        top_color = (10, 8, 20)
        mid_color = (22, 10, 46)
        bottom_color = (10, 18, 30)
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        # Stars
        for x, y, s in self.stars:
            pygame.draw.circle(surface, (180, 200, 255), (int(x), int(y)), 1)
        # City silhouettes
        ground_y = int(self.height * 0.82)
        for li, layer in enumerate(self.layers):
            color = self.layer_colors[li]
            for x, w, h, _, _ in layer:
                rect = pygame.Rect(int(x), int(ground_y - h), int(w), int(h))
                pygame.draw.rect(surface, color, rect)
                # neon windows as small bright dots
                if li >= 1:
                    for _ in range(4 if li == 1 else 7):
                        wx = rect.left + random.randint(4, max(5, rect.width - 5))
                        wy = rect.top + random.randint(4, max(5, rect.height - 5))
                        pygame.draw.rect(surface, (255, 80, 150), (wx, wy, 2, 2))
        # Ground glow line
        pygame.draw.line(surface, (255, 20, 120), (0, ground_y), (self.width, ground_y), 2)
        # Neon rain
        for x, y, s, L in self.rain:
            pygame.draw.line(surface, (120, 255, 170), (int(x), int(y)), (int(x - 2), int(y - L)), 1)
        # Scanlines overlay
        surface.blit(self.scan_surface, (0, 0))
...
def run() -> None:
    ...
    # Animated background (persists across matches)
    bg = CyberpunkBackground(WINDOW_WIDTH, WINDOW_HEIGHT)
    ...
    while True:
        ...
        shop_page = 0
        paused = False
        resume_timer = 0.0  # 3s countdown after unpausing
        ...
        while running:
                    ...
                    if admin_panel_open:
                        if event.key == pygame.K_ESCAPE:
                            ...
                            if event.key == pygame.K_1:
                                ...
                                player.bullet_size_level = 0
                                player.bullet_pierce_level = 0
                                player.magnet_radius = 60.0
                                player.max_health_bonus = 0
                                player.armor = 0
                                player.bullet_damage_level = 0
                                player.coin_gain_level = 0
                                player.dodge_level = 0
                                player.regen_level = 0
                                player.lifesteal_level = 0
                                player.crit_level = 0
                                player.aoe_on_kill_level = 0
                                player.magnet_bonus_level = 0
                                player.bullet_speed_level = 0
                                player.bullet_speed_multiplier = 1.0
                                player.health = player.max_health()

                                # reset shop progress vars
                                speed_levels = 0
                                firerate_levels = 0
                                pierce_levels = 0
                                size_levels = 0
            ...
            # Update
            if not paused and resume_timer <= 0 and not shop_open and not item_shop_open and not console_open and not admin_panel_open and player.is_alive():
            ...
            if not paused and resume_timer <= 0 and not shop_open and not item_shop_open and not console_open and not admin_panel_open and player.is_alive():
                ...
                if explosions:
                    new_alive: list[Enemy] = []
                    for e in enemies:
                        if not e.is_boss or e.health > 0:
                            new_alive.append(e)
                    enemies = new_alive
                    for exp in explosions:
                        for e in enemies:
                            # prevent multiple hits by same explosion
                            eid = id(e)
                            if eid in exp.hit_ids:
                                continue
                            if circle_intersects_circle(exp.position, exp.radius, e.position, e.radius):
                                exp.hit_ids.add(eid)
                                e.health -= exp.damage
                    # cleanup dead enemies from explosions
                    remaining: list[Enemy] = []
                    for e in enemies:
                        if e.health <= 0:
                            score += 1
                            player.on_kill()
                            # chain explosions allowed
                            if player.aoe_on_kill_level > 0:
                                explosion_radius = 40 + 10 * player.aoe_on_kill_level
                                explosions.append(Explosion(
                                    position=Vector2(e.position),
            ...
            if item_shop_open:
                ...
                for idx, (label, level, desc, base_cost) in enumerate(items2):
                    ...
                    btn_bg = (46, 52, 60) if afford else (36, 40, 46)
                    if r.collidepoint(pygame.mouse.get_pos()):
                        btn_bg = (56, 62, 70) if afford else (44, 48, 54)
                    pygame.draw.rect(screen, btn_bg, r, border_radius=8)
            ...
            # Resume countdown overlay (when coming back from pause)
            if not paused and resume_timer > 0:
                resume_timer = max(0.0, resume_timer - dt)
                overlay = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SRCALPHA)
                overlay.fill((0, 0, 0, 100))
                screen.blit(overlay, (0, 0))
                sec_left = int(math.ceil(resume_timer))
                msg = big_font.render(str(sec_left), True, UI_ACCENT)
